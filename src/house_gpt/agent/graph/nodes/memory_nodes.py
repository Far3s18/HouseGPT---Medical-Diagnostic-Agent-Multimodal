...
async def memory_injection_node(state: AIHouseState):
    ...
    recent_context = " ".join([m.content for m in state["messages"][-3:] if hasattr(m, 'content')])
    ...
    logger.debug(f"[TIMER] inject_node: {time.time() - t:.2f}s | memories_found={len(memories)}")
